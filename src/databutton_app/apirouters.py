...
from .deprecated_baserouter import router as default_router
...
from .pathutils import read_router_config, find_submodules, convert_exception_to_model
...
def import_submodules(
    ...
    for name in submodules:
        ...
        # Check for router reuse
        if user_router is default_router:
            result.errors.append(
                f"In {name}: importing router from databutton_app is deprecated, use APIRouter from fastapi directly"
            )
        elif any(user_router is r for r in routers.values()):
            result.errors.append(
...
def add_uniqueness_check_errors(import_results: list[ImportResult]):
    ...
    for result in import_results:
        for ep in result.endpoints:
            ...
            # to the same function in the generated brain client)
